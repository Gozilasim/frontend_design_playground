...
export default function NginxBlueGreen({ viewport }: DesignProps) {
  ...
  const isVertical =
    viewport.deviceType === 'mobile' || viewport.deviceType === 'shorts';

  // Sound effects generator using Web Audio API
  const playSound = useCallback(
    (type: 'click' | 'reload' | 'success') => {
      try {
  ...
  return (
    <div className="flex h-full w-full flex-col justify-center items-center bg-[#070913] text-slate-100 overflow-hidden font-sans p-4 relative select-none">
      ...
      {/* Header Panel */}
      <div className="flex items-center justify-between border-b border-white/5 pb-2 mb-2 flex-shrink-0">
        <div>
          <h1 className="text-base font-bold tracking-tight bg-gradient-to-r from-emerald-400 via-sky-400 to-blue-400 bg-clip-text text-transparent">
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className={cn(
              'flex items-center gap-1.5 px-2.5 py-1 rounded-lg border border-white/10 text-[10px] font-semibold backdrop-blur-sm transition-all',
              isPlaying
          ...
          <button
            ...
            className="p-1 rounded-lg border border-white/10 bg-white/5 hover:bg-white/10 transition-all text-slate-300"
            title="Reset"
          >
      ...
      <div className="flex flex-col rounded-2xl border border-white/5 bg-slate-950/35 backdrop-blur-md p-4 mb-3 flex-shrink-0 select-none overflow-hidden">
        {/* Node Circles and Progress Line Row */}
        <div className="relative flex items-center justify-between px-10 h-10">
          {/* Connection Line Container (Spans exactly from Node 1 center to Node 3 center) */}
          <div className="absolute left-[88px] right-[88px] h-[2.5px] top-1/2 -translate-y-1/2 pointer-events-none z-0">
            {/* Background Line */}
          ...
          {[
            {
              ...
              icon: Server,
              color: 'emerald',
              activeClass: 'border-emerald-500 text-emerald-400 bg-emerald-950/80 shadow-[0_0_15px_rgba(16,185,129,0.4)]',
            },
            {
              step: 1 as const,
              icon: RefreshCw,
              color: 'amber',
              activeClass: 'border-amber-500 text-amber-400 bg-amber-950/80 shadow-[0_0_15px_rgba(245,158,11,0.4)]',
            },
            {
              step: 2 as const,
              icon: CheckCircle2,
              color: 'blue',
              activeClass: 'border-blue-500 text-blue-400 bg-blue-950/80 shadow-[0_0_15px_rgba(59,130,246,0.4)]',
            },
          ].map((item) => {
            ...
            return (
              <button
                ...
                className="relative flex justify-center w-24 focus:outline-none z-10"
              >
                <div
                  className={cn(
                    'w-9 h-9 rounded-full border-2 flex items-center justify-center transition-all duration-500 bg-slate-950',
                    isActive
                      ? item.activeClass
                      : isCompleted
                ...
                >
                  {isActive && (
                    <span
                      className={cn(
                        'absolute -inset-1 rounded-full border border-current opacity-35 animate-ping pointer-events-none',
          ...
          {[
            ...
            return (
              <button
                key={item.step}
                onClick={() => handleStepSelect(item.step)}
                className="flex flex-col items-center text-center w-24 focus:outline-none group"
              >
                <span
                  className={cn(
                    'text-[9.5px] font-bold transition-colors duration-300 leading-tight',
                    isActive
                      ? item.textActive
                      : isCompleted
                        ? 'text-slate-300'
                ...
                <span className="text-[7.5px] text-slate-500 font-mono mt-0.5 block truncate max-w-full">
                  {item.desc}
                </span>
              </button>
            );
      ...
      <div
        className={cn(
          'relative rounded-2xl border border-white/5 bg-slate-950/40 select-none overflow-hidden flex items-center justify-center flex-grow',
          isVertical ? 'min-h-[340px] max-h-[480px]' : 'min-h-[300px] max-h-[420px]'
        )}
      >
        ...
        >
              ...
              {activeStep === 0 &&
                particles.slice(0, 5).map((p) => (
                  <circle
                    ...
                    r="3"
                    fill="#10b981"
                    className="filter drop-shadow-[0_0_5px_#10b981]"
                  >
                    <animateMotion
                      dur="1.6s"
                      repeatCount="indefinite"
            ...
            <foreignObject x="20" y="50" width="140" height="200">
              <div className="flex flex-col gap-3 h-full justify-center">
                {['Web User', 'Mobile App', 'API Client'].map((label, idx) => (
                  <div
                    key={idx}
                    className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-slate-900/90 border border-white/5 shadow-inner"
                  >
            ...
            <foreignObject x="10" y="15" width="340" height="40">
              <div className="flex items-center justify-around h-full">
                {['Web User', 'Mobile App', 'API Client'].map((label, idx) => (
                  <div
                    key={idx}
                    className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-slate-900/90 border border-white/5 shadow-inner"
                  >
                    <div className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-pulse" />
          ...
          >
            <div
              className={cn(
                'flex flex-col items-center justify-center h-full rounded-2xl border bg-slate-900/90 backdrop-blur-md transition-all duration-300 shadow-xl',
                isReloading
                  ? 'border-amber-500/60 scale-105 shadow-amber-500/10'
                  : 'border-white/10'
              )}
            >
              {isReloading && (
                <div className="absolute inset-0 rounded-2xl border-2 border-amber-500/60 animate-ping opacity-60 pointer-events-none" />
              )}
          ...
          >
            <div
              className={cn(
                'flex flex-col justify-center px-3 h-full rounded-xl border bg-slate-900/95 transition-all duration-500',
                activeStep === 0
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="px-1.5 py-0.2 rounded text-[7.5px] font-bold bg-emerald-500/10 text-emerald-400 font-mono">
                  v1.0.0
                </span>
              </div>
              <span className="text-[9.5px] font-bold text-slate-200 mt-1">
                Green Cluster
              </span>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="px-1.5 py-0.2 rounded text-[7.5px] font-bold bg-blue-500/10 text-blue-400 font-mono">
                  v2.0.0
                </span>
              </div>
              <span className="text-[9.5px] font-bold text-slate-200 mt-1">
